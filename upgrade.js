...
document.addEventListener('DOMContentLoaded', () => {
          ...
          <div class="modal-content tutorial-modal-content wide-modal vm-customization-modal">
            ...
            <div class="customizer-section">
              ...
              <div class="toggle-grid atmosphere-control-grid">
                <label class="toggle-card select-card"><span>Hiệu ứng</span><select id="weatherEffectSelect" class="modern-input"><option value="off">Tắt</option><option value="rain">Rain</option><option value="snow">Snow</option></select></label>
  ...
  function bindEvents() {
    ...
    byId('weatherEffectSelect')?.addEventListener('change', async (event) => {
      state.ui.weatherEffect = ['off', 'rain', 'snow'].includes(event.target.value) ? event.target.value : 'off';
      applyUiPreferences();
      await saveUiState();
    });
  ...
  function normalizeUiState(raw = {}) {
    const ui = {
      ...
      weatherEffect: ['off', 'rain', 'snow'].includes(raw.weatherEffect) ? raw.weatherEffect : 'off',
